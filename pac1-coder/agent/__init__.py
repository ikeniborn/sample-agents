...
from .classifier import ModelRouter, TASK_CODER, TASK_PREJECT
...
def _inject_addendum(base_prompt: str, addendum: str) -> str:
    ...
    End-append keeps the addendum after all structural rules, preventing
    the model from over-weighting task-specific instructions relative to
    core rules (which caused extra steps and timeouts when injected mid-prompt).
    """
    if not addendum:
        return base_prompt
...
def run_agent(router: ModelRouter, harness_url: str, task_text: str) -> dict:
    ...
    4. build_dynamic_addendum() — calls a lightweight LLM to generate task-specific
       ...
       Token budget: PROMPT_BUILDER_MAX_TOKENS (default 500).
    ...
    # Fast-path: preject tasks skip prompt_builder addendum (1-step loop → immediate OUTCOME_NONE_UNSUPPORTED)
    if task_type == TASK_PREJECT:
        ...
        stats = run_loop(vm, model, task_text, pre, cfg, task_type=task_type,
                         coder_model=model, coder_cfg=cfg,
                         evaluator_model=model, evaluator_cfg=cfg)

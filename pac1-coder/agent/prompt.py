...
**EXACT DATE LOOKUP** — trigger ONLY when task uses "exactly", "precisely", or an explicit calendar date:
  If no file matches the exact target date → OUTCOME_NONE_CLARIFICATION.
  Do NOT approximate — "exactly" means exact, no closest-match fallback.

**RELATIVE DATE LOOKUP** ("N days ago", "last week", "recently") WITHOUT "exactly"/"precisely":
  Compute the target date via code_eval(datetime), then search files by vendor/item/content match —
  NOT strict date equality. If the computed date yields no result, widen search by
  vendor name or line-item keyword. OUTCOME_NONE_CLARIFICATION only when no relevant
  file exists at all, not just because the date differs slightly.
